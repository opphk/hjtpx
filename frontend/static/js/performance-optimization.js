(function() {
  ...
  const PERFORMANCE_CONFIG = {
    ...
    enableWebP: true,
    enableAvif: true,
    qualitySteps: [0.9, 0.75, 0.6, 0.45],
  ...
  class PerformanceOptimizer {
    ...
    detectImageFormats() {
      const formats = { webp: false, avif: false, jpegxl: false };
      
      const webpTest = new Image();
      webpTest.onload = () => { formats.webp = true; };
      webpTest.onerror = () => { formats.webp = false; };
      webpTest.src = 'data:image/webp;base64,UklGRjoAAABXVRYQVzIAAAAUAAsyAAFfAAEA';

      const avifTest = new Image();
      avifTest.onload = () => { formats.avif = true; };
      avifTest.onerror = () => { formats.avif = false; };
      avifTest.src = 'data:image/avif;base64,';

      return formats;
    }

    init() {
      console.log('[Performance] 初始化性能优化模块 v2');
    ...
    setupInlineCriticalCss() {
      ...
      const criticalCss = `
        ...
        .perf-blur-up { filter: blur(8px); transition: filter 0.5s ease; }
        .perf-blur-up.loaded { filter: blur(0); }
        .perf-lazy-placeholder {
      ...
      if (!window.requestIdleCallback) {
        window.requestIdleCallback = function(cb, options) {
          ...
          return setTimeout(() => cb({ 
            timeRemaining: () => Math.max(0, timeout - (Date.now() - performance.now())) 
          }), 1);
        };
    ...
    setupLazyLoading() {
      if ('IntersectionObserver' in window) {
        ...
        const lazyElements = document.querySelectorAll('[data-lazy], [data-src], [data-bg], img[data-srcset], img.lazy');
    ...
    loadLazyElement(element) {
      if (element.hasAttribute('data-src')) {
        ...
        if (!this.loadedImages.has(src)) {
          const webpSrc = this.getOptimizedImageUrl(src);
          element.src = webpSrc || src;
          element.removeAttribute('data-src');
          this.loadedImages.add(src);
          element.classList.add('lazy-loaded');
          this.performanceMetrics.lazyLoadedCount++;
        }
      }

      if (element.hasAttribute('data-bg')) {
        const bg = element.getAttribute('data-bg');
        const webpBg = this.getOptimizedImageUrl(bg);
        element.style.backgroundImage = `url(${webpBg || bg})`;
      ...
      if (element.hasAttribute('data-srcset')) {
        ...
        if (srcset) {
          element.srcset = this.optimizeSrcset(srcset);
      ...
      if (element.hasAttribute('data-lazy')) {
        ...
        try {
          ...
          if (content.src) {
            element.src = this.getOptimizedImageUrl(content.src) || content.src;
          }
        } catch (e) {
    ...
    getOptimizedImageUrl(url) {
      if (!PERFORMANCE_CONFIG.enableWebP || !this.supportedFormats.webp) {
        return null;
      }
      ...
      if (!supportedExts.includes(ext.toLowerCase())) return null;
    ...
    optimizeSrcset(srcset) {
      ...
      return srcset.replace(/\.(jpg|jpeg|png)\s/g, '.webp ');
    }

    setupImageOptimization() {
      const images = document.querySelectorAll('img[data-optimize], img[loading="lazy"], img:not([data-optimized])');
    ...
    optimizeImage(img) {
      ...
      if (img.hasAttribute('data-optimized') && img.getAttribute('data-optimized') === 'true') {
        return;
      }

      if (this.compressedImages.has(src)) {
        const cached = this.compressedImages.get(src);
      ...
      if (newWidth === width && newHeight === height && !this.needsCompression(img)) {
        img.setAttribute('data-optimized', 'true');
        return;
      }
      ...
      const quality = this.getOptimalQuality(img);
      const optimizedSrc = canvas.toDataURL(`image/${format}`, quality);

      const originalSize = this.getOriginalSize(src, width, height);
      const compressedSize = optimizedSrc.length * 0.75;
      ...
      if (bytesSaved > 0) {
        ...
        this.compressedImages.set(src, { dataUrl: optimizedSrc, quality: quality, format: format });
        this.performanceMetrics.imagesCompressed++;
        this.performanceMetrics.bytesSaved += bytesSaved;
        console.log(`[Performance] 图片压缩: ${width}x${height} -> ${newWidth}x${newHeight}, 格式: ${format}, 质量: ${quality.toFixed(2)}, 节省: ${this.formatBytes(bytesSaved)}`);
      }
    ...
    getBestFormat() {
      if (PERFORMANCE_CONFIG.enableAvif && this.supportedFormats.avif) {
        return 'avif';
      }
      if (PERFORMANCE_CONFIG.enableWebP && this.supportedFormats.webp) {
        return 'webp';
    ...
    getOptimalQuality(img) {
      const sizeThresholds = [50000, 150000, 300000, 500000];
      const qualities = PERFORMANCE_CONFIG.qualitySteps;
      
      const estimatedSize = (img.naturalWidth || img.width) * (img.naturalHeight || img.height) * 4;
      
      for (let i = 0; i < sizeThresholds.length; i++) {
        if (estimatedSize < sizeThresholds[i]) {
          return qualities[i];
        }
      }
    ...
    needsCompression(img) {
      ...
      return (img.naturalWidth || img.width) * (img.naturalHeight || img.height) * 4 > threshold;
    }

    getOriginalSize(src, width, height) {
      const avgBytesPerPixel = 2.4;
      return width * height * avgBytesPerPixel;
    }
    ...
    setupBlurUpLoading() {
      ...
      images.forEach(img => {
        ...
        if (src) {
          img.classList.add('perf-blur-up');
    ...
    generateSrcset(src) {
      ...
      return sizes.map(size => {
        ...
        const formatExt = this.supportedFormats.webp ? '.webp' : ext;
        return `${base}${suffix}${formatExt} ${size}x`;
      }).join(', ');
    }

    generateSizes(img) {
      const width = img.getAttribute('width') || img.style.width || '100%';
      return `(max-width: 576px) 100vw, (max-width: 768px) 50vw, ${width}`;
    }
    ...
    setupMemoryWarningHandler() {
      ...
      if ('webkitMemoryPressureListener' in window) {
        window.webkitMemoryPressureListener.onmemorypressure = (level) => {
          if (level === 2) {
            this.clearMemoryCache();
          }
        };
    ...
    cleanupUnusedImages() {
      ...
      images.forEach(img => {
        ...
        const isOffscreen = rect.top > viewportHeight || rect.bottom < -viewportHeight;
      ...
      if ('serviceWorker' in navigator) {
        navigator.serviceWorker.ready.then(registration => {
          registration.active?.postMessage({
            type: 'CACHE_CONFIG',
            config: {
              cacheTTL: PERFORMANCE_CONFIG.cacheTTL,
              enableWebP: PERFORMANCE_CONFIG.enableWebP,
              quality: PERFORMANCE_CONFIG.compressionQuality
            }
          });
        });
      }
    ...
    getPerformanceReport() {
      return {
        ...
        bytesSavedFormatted: this.formatBytes(this.performanceMetrics.bytesSaved)
      };
    }
  }
