...
// WebhookChannel webhook渠道
type WebhookChannel struct {
	URL     string
	Headers map[string]string
}

func (wc *WebhookChannel) Send(msg AlertMessage) error {
	return nil
}
...
func (ec *EmailChannel) Send(msg AlertMessage) error {
	return nil
}
...
func (sc *SlackChannel) Send(msg AlertMessage) error {
	return nil
}
...
func (dc *DingTalkChannel) Send(msg AlertMessage) error {
	return nil
}
...
func CreateChannel(channelType string, config map[string]interface{}) (AlertChannel, error) {
	...
	case "email":
		return &EmailChannel{
			SMTPHost: getStringConfig(config, "smtp_host", "localhost"),
			SMTPPort: getIntConfig(config, "smtp_port", 587),
			Username: getStringConfig(config, "username", ""),
			Password: getStringConfig(config, "password", ""),
			From:     getStringConfig(config, "from", ""),
			To:       getStringArrayConfig(config, "to"),
		}, nil
	case "slack":
		return &SlackChannel{
			WebhookURL: getStringConfig(config, "webhook_url", ""),
			Channel:    getStringConfig(config, "channel", ""),
			Username:   getStringConfig(config, "username", "Alert Bot"),
		}, nil
	case "dingtalk":
...
func getStringArrayConfig(config map[string]interface{}, key string) []string {
...
// EnableRule 启用告警规则
func (as *AlertService) EnableRule(id uint) error {
	if as.db == nil {
		return nil
	}
	if err := as.db.Model(&models.AlertRule{}).Where("id = ?", id).Update("is_enabled", true).Error; err != nil {
		return err
	}
	return as.LoadRules()
}
...
// DisableRule 禁用告警规则
func (as *AlertService) DisableRule(id uint) error {
	if as.db == nil {
		return nil
	}
	if err := as.db.Model(&models.AlertRule{}).Where("id = ?", id).Update("is_enabled", false).Error; err != nil {
		return err
	}
	return as.LoadRules()
}
...
// MuteAlerts 静默告警
func (as *AlertService) MuteAlerts(ruleID uint, duration time.Duration) error {
	if as.db == nil {
		return nil
	}
	endTime := time.Now().Add(duration)
	return as.db.Model(&models.AlertRule{}).Where("id = ?", ruleID).Updates(map[string]interface{}{
		"is_enabled":     false,
		"silence_until": &endTime,
	}).Error
}

// GetAlertCounts 获取告警计数
func (as *AlertService) GetAlertCounts(status string) (map[string]int64, error) {
	...
	var total int64
	as.db.Model(&models.AlertRecord{}).Where("status = ?", status).Count(&total)
	counts["total"] = total

	var critical int64
	as.db.Model(&models.AlertRecord{}).Where("status = ? AND severity = ?", status, "critical").Count(&critical)
	counts["critical"] = critical

	var warning int64
	as.db.Model(&models.AlertRecord{}).Where("status = ? AND severity = ?", status, "warning").Count(&warning)
