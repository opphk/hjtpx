...
type VRGestureConfig struct {
	...
	Hand               string         `json:"hand"` // left, right, either
}
...
func (s *VRGeneratorService) Verify(ctx context.Context, req *VRVerifyRequest) (*VRVerifyResponse, error) {
...
func (s *VRGeneratorService) evaluateInteraction(req *VRVerifyRequest, session *VRSession) float64 {
	totalScore := 0.0
	weightSum := 0.0
	...
	if req.Interaction != nil && req.Interaction.ObjectPositions != nil {
		for _, target := range session.VRConfig.Targets {
			...
			distance := calculateDistance3D(pos, target.Position)
	...
	if req.Interaction != nil && req.Interaction.ObjectRotations != nil {
		for _, obj := range session.VRConfig.Objects {
			...
			angleDiff := calculateAngleDifference(rot, obj.TargetRotation)
	...
	if req.EyeData != nil {
		eyeScore := s.evaluateEyeTracking(req.EyeData, session)
...
func (s *VRGeneratorService) evaluateEyeTracking(eyeData *VREyeTrackingData, session *VRSession) float64 {
...
func (s *VRGeneratorService) calculateAccuracy(req *VRVerifyRequest, session *VRSession) float64 {
...
func (s *VRGeneratorService) calculateBehaviorScore(behaviorData map[string]interface{}) float64 {
	score := 0.5
...
func (s *VRGeneratorService) generateFeedback(req *VRVerifyRequest, session *VRSession) string {
...
func (s *VRGeneratorService) generateAnalytics(req *VRVerifyRequest, session *VRSession) *VRAnalytics {
