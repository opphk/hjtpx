...
func (gen *OptimizedImageGenerator) GenerateSliderImage(width, height int) ([]byte, int, int) {
	...
	pieceBuf := bytes.Buffer{}
	encoder.Encode(&pieceBuf, pieceImg)
...
type ImageGenerationStats struct {
...
var imageStats = &ImageGenerationStats{}
var statsMu sync.RWMutex

func RecordImageGeneration(pixels int, duration time.Duration) {
	statsMu.Lock()
	defer statsMu.Unlock()

	imageStats.TotalGenerated++
	imageStats.TotalPixels += int64(pixels)

	if imageStats.TotalGenerated == 1 {
		imageStats.MinGenTime = duration
		imageStats.MaxGenTime = duration
		imageStats.AvgGenTime = duration
	} else {
		total := imageStats.AvgGenTime * time.Duration(imageStats.TotalGenerated-1)
		imageStats.AvgGenTime = (total + duration) / time.Duration(imageStats.TotalGenerated)

		if duration < imageStats.MinGenTime {
			imageStats.MinGenTime = duration
		}
		if duration > imageStats.MaxGenTime {
			imageStats.MaxGenTime = duration
		}
	}
}

func GetImageGenerationStats() *ImageGenerationStats {
	statsMu.RLock()
	defer statsMu.RUnlock()
	return &ImageGenerationStats{
		TotalGenerated: imageStats.TotalGenerated,
		TotalPixels:    imageStats.TotalPixels,
		AvgGenTime:     imageStats.AvgGenTime,
		MaxGenTime:     imageStats.MaxGenTime,
		MinGenTime:     imageStats.MinGenTime,
	}
}
