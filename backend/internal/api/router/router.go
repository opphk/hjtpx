...
func SetupRouter() *gin.Engine {
	...
	adminRouter := r.Group("/admin")
	{
		...
		adminRouter.GET("/api/heatmap", handler.GetDashboardHeatmapData)
		adminRouter.GET("/api/radar", handler.GetDashboardRadarData)
		adminRouter.GET("/api/sankey", handler.GetDashboardSankeyData)
		adminRouter.GET("/api/advanced-analytics", handler.GetAdvancedAnalytics)
		adminRouter.GET("/api/custom-range", handler.GetCustomTimeRangeData)
		...
		adminRouter.GET("/api/logs/stats/analysis", handler.GetLogAnalysisStats)
		adminRouter.GET("/api/logs/stats/trend", handler.GetLogTrendStats)
		adminRouter.POST("/api/logs/advanced-search", handler.AdvancedSearchLogs)
		adminRouter.POST("/api/logs/save-search", handler.SaveLogSearch)
		adminRouter.GET("/api/logs/saved-searches", handler.GetSavedLogSearches)
		adminRouter.DELETE("/api/logs/saved-searches/:id", handler.DeleteSavedLogSearch)
	...
	api := r.Group("/api/v1")
	{
		...
		admin.Use(middleware.AuthMiddleware())
		{
			...
			admin.POST("/batch-operations/:id/rollback", handler.RollbackBatchOperation)
			admin.POST("/batch-operations/:id/cancel", handler.CancelBatchOperation)
			admin.POST("/batch/blacklist/import", handler.BlacklistBatchImport)
			admin.POST("/batch/blacklist/import-sync", handler.BlacklistImportSync)
			admin.POST("/batch/blacklist/delete", handler.BlacklistBatchDelete)
			admin.POST("/batch/application/config", handler.ApplicationBatchConfig)
			admin.POST("/batch/rules/update", handler.RuleBatchUpdate)
			admin.GET("/batch/blacklist/template", handler.DownloadBlacklistTemplate)
