...
router.get('/websocket/health', (req, res) => {
  try {
    ...
    const errorRate =
      metrics.totalConnections > 0 ? (metrics.errors / metrics.totalConnections) * 100 : 0;
...
router.get('/websocket/performance', (req, res) => {
  try {
    ...
    const avgMessageLatency =
      metrics.messagesReceived > 0 ? (metrics.messagesSent / metrics.messagesReceived) * 100 : 0;

    const messagesPerSecond =
      metrics.uptime > 0 ? (metrics.messagesSent / (metrics.uptime / 1000)).toFixed(2) : 0;

    const connectionsPerSecond =
      metrics.uptime > 0 ? (metrics.totalConnections / (metrics.uptime / 1000)).toFixed(2) : 0;
