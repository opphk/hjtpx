...
- Name: register.spec.js >> User Registration Flow >> Form Validation >> should show validation error for password mismatch
- Location: tests/e2e/register.spec.js:140:5
...
Call log:
  ...
  - waiting for locator('.error-message, [class*="error"]').first()
    14 × locator resolved to <input required="" type="password" aria-invalid="true" id="confirmPassword" aria-required="true" placeholder="请再次输入密码" aria-disabled="false" name="confirmPassword" value="DifferentPassword123" class="form-input input-error" aria-describedby="confirmPassword-error"/>
       - unexpected value ""
