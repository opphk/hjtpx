...
- Name: register.spec.js >> User Registration Flow >> Form Validation >> should show validation error for short password
- Location: tests/e2e/register.spec.js:96:5
...
Call log:
  ...
  - waiting for locator('.error-message, [class*="error"]').first()
    14 × locator resolved to <input required="" id="password" value="short" type="password" name="password" placeholder="请输入密码" aria-invalid="true" aria-required="true" aria-disabled="false" class="form-input input-error" aria-describedby="password-error"/>
       - unexpected value ""
