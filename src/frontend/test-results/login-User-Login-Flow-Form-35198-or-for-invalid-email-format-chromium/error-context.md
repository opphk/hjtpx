...
- Name: login.spec.js >> User Login Flow >> Form Validation >> should show validation error for invalid email format
- Location: tests/e2e/login.spec.js:71:5
...
Call log:
  ...
  - waiting for locator('.error-message, [class*="error"]').first()
    14 × locator resolved to <input id="email" type="text" required="" name="email" placeholder="请输入邮箱" aria-invalid="true" aria-required="true" aria-disabled="false" value="invalid-email" class="form-input input-error" aria-describedby="email-error"/>
       - unexpected value ""
