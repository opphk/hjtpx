...
- Name: register.spec.js >> User Registration Flow >> Form Validation >> should show validation error for password without uppercase
- Location: tests/e2e/register.spec.js:107:5
...
Expected pattern: /密码|大写|uppercase/i
Received string:  ""
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.error-message, [class*="error"]').first()
    14 × locator resolved to <input required="" id="password" type="password" name="password" placeholder="请输入密码" aria-invalid="true" aria-required="true" aria-disabled="false" value="validpassword123" class="form-input input-error" aria-describedby="password-error"/>
       - unexpected value ""
